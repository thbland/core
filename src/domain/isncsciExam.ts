...
export class IsncsciExam {
    ...
    public static nonSciImpairmentRegex: RegExp = /.+[!]/;
    public static nonSciImpairmentFlagsRegex: RegExp = /[\\*!]/;
    public static normalMotorValue: number = 5;
    public static normalSensoryValue: number = 2;
    ...
    private static getIsncsciExamLevels(): NeuroLevel[] {
        const levels: NeuroLevel[] = [];
        const c1: NeuroLevel = new NeuroLevel();
        c1.name = 'C1';
        c1.previous = null;
        ...
        let previousLevel: NeuroLevel = c1;
        const levelNames: string[] = ['C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8',
                                    'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12',
                                    'L1', 'L2', 'L3', 'L4', 'L5', 'S1', 'S2', 'S3', 'S4_5'];

        const keyMuscles: object = {
            C5: true, C6: true, C7: true, C8: true, L2: true, L3: true, L4: true, L5: true, S1: true, T1: true,
        };

        for (let i: number = 0; i < levelNames.length; i++) {
            const name: string = levelNames[i];

            const currentLevel =  new NeuroLevel();
     ...
     * It takes into consideration specific elements like asterisks, exclamation signs, or the use of NT.
     *
    ...
    private static getDermatomeValue(text: string, normalValue: number): number {
        ...
        const value = parseInt(text.replace(IsncsciExam.nonSciImpairmentFlagsRegex, ''), 10);

        return isNaN(value) ? 0 : value;
    }

    /**
     * Sets the right names and numeric values for the specified
     * neurology level based on the text based entries provided.
     *
    ...
    private static updateLevel(
        level: NeuroLevel, rightTouch: string, leftTouch: string,
    ...
    public analContraction: BinaryObservation | null = null;
    public analSensation: BinaryObservation | null = null;
    public rightLowestNonKeyMuscleWithMotorFunction: NeuroLevel | null = null;
    public leftLowestNonKeyMuscleWithMotorFunction: NeuroLevel | null = null;
    ...
    public getLevelAt(ordinal): NeuroLevel {
        ...
        // Remember that we have C1 at index 0 and clinicians are not used to thinking about it when calculating.
     ...
     * You can also use the exclamation mark and asterisk at the end of the string to indicate
     * impairment not due to a spinal cord injury.
     ...
     */
    public updateLevelByName(
        levelName: string, rightTouch: string, leftTouch: string, rightPrick: string,
        leftPrick: string, rightMotor: string, leftMotor: string): IsncsciExam {
        const level = this.getLevelWithName(levelName);

        if (!level) {
            // tslint:disable-next-line:max-line-length
            throw new Error(`${IsncsciExam.is} : updateLevelByName : Unable to find a neuro level with name ${levelName}`);
        }
