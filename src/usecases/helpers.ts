 ...
 * @returns {string} An empty string when the combination is valid,
 *                   or 'invalid-dermatome-name' when the name is invalid.
 */
export function validateDermatomeName(dermatomeName: string): string {
    ...
    return validMotorNameRegExp.test(dermatomeName)
        || validSensoryNameRegExp.test(dermatomeName) ? '' : 'invalid-dermatome-name';
}
 ...
 * @param {string} value
 * @returns {string} An empty string when the combination is valid,
 *                   or one of the following values when the combination is invalid:
 *                   ['invalid-dermatome-name', 'invalid-motor-value', 'invalid-sensory-value']
 */
