...
import { Algorithm, BinaryObservation, IsncsciExam, IsncsciTotals } from '../../domain';
import { iIsncsciExamModel } from './iIsncsciExamModel';
import { motorLevelNameRegExp, validMotorValueRegExp, validSensoryValueRegExp } from './regularExpressions';
...
export class CalculateTotalsUseCase {
    static get is(): string { return "rhi-core-isncsci-usecases.CalculateTotalsUseCase"; }
    ...
    * 1. The clinician provides the values required for a full exam.
    */
    public execute(examModel: iIsncsciExamModel): void {
        // 2. the system validates the exam.
        const isncsciExam: IsncsciExam = CalculateTotalsUseCase.getIsncsciExamFrom(examModel);
        // 3. The system runs the exam through the algorithm.
        const isncsciTotals: IsncsciTotals = Algorithm.getTotalsFor(isncsciExam);
        // 4. The system updates the application state with the generated totals.
        this.appStoreProvider.setTotals(isncsciTotals);
    }
     ...
     */
    private static getIsncsciExamFrom(examData: iIsncsciExamModel): IsncsciExam {
        ...
        isncsciExam.analContraction = CalculateTotalsUseCase.getBinaryObservationFor(examData.analContraction);
        isncsciExam.analSensation = CalculateTotalsUseCase.getBinaryObservationFor(examData.analSensation);
        ...
        keyMap.forEach(
            (key: { prefix: string, start: number, end: number }) => {
                for (let i:number = key.start; i<=key.end; i++) {
                    CalculateTotalsUseCase.updateLevelByName(
                        `${key.prefix}${i}`,
                        isncsciExam,
                        examData
                    );
     ...
     */
    private static updateLevelByName(levelName: string, isncsciExam: IsncsciExam, examData: iIsncsciExamModel): void {
     ...
     */
    private static getBinaryObservationFor(value: string): BinaryObservation {
