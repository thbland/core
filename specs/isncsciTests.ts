/*
@license
Copyright (c) 2015 Rick Hansen Institute. All rights reserved.
This code may only be used under the modified Apache license found at https://raw.githubusercontent.com/EddieMachete/rhi-core-isncsci-algorithm/master/LICENSE
Author: RhiTech <tech@rickhanseninstitute.org>
*/

'use strict';

export interface iDermatomeModel {
  rightTouch:string,
  leftTouch:string,
  rightPrick:string,
  leftPrick:string
}

export interface iDermatomeWithMotorModel {
  rightTouch:string,
  leftTouch:string,
  rightPrick:string,
  leftPrick:string,
  rightMotor:string,
  leftMotor:string
}

export interface iDermatomesModel {
  C2:iDermatomeModel,
  C3:iDermatomeModel,
  C4:iDermatomeModel,
  C5:iDermatomeWithMotorModel,
  C6:iDermatomeWithMotorModel,
  C7:iDermatomeWithMotorModel,
  C8:iDermatomeWithMotorModel,
  T1:iDermatomeWithMotorModel,
  T2:iDermatomeModel,
  T3:iDermatomeModel,
  T4:iDermatomeModel,
  T5:iDermatomeModel,
  T6:iDermatomeModel,
  T7:iDermatomeModel,
  T8:iDermatomeModel,
  T9:iDermatomeModel,
  T10:iDermatomeModel,
  T11:iDermatomeModel,
  T12:iDermatomeModel,
  L1:iDermatomeModel,
  L2:iDermatomeWithMotorModel,
  L3:iDermatomeWithMotorModel,
  L4:iDermatomeWithMotorModel,
  L5:iDermatomeWithMotorModel,
  S1:iDermatomeWithMotorModel,
  S2:iDermatomeModel,
  S3:iDermatomeModel,
  S4_5:iDermatomeModel
}

export interface iTotalsModel {
  rightUpperMotorTotal:string,
  rightUpperMotorContainsNt:boolean,
  leftUpperMotorTotal:string,
  leftUpperMotorContainsNt:boolean,
  rightLowerMotorTotal:string,
  rightLowerMotorContainsNt:boolean,
  leftLowerMotorTotal:string,
  leftLowerMotorContainsNt:boolean,
  rightTouchTotal:string,
  rightTouchContainsNt:boolean,
  leftTouchTotal:string,
  leftTouchContainsNt:boolean,
  rightPrickTotal:string,
  rightPrickContainsNt:boolean,
  leftPrickTotal:string,
  leftPrickContainsNt:boolean,
  rightMotorTotal:string,
  leftMotorTotal:string,
  upperMotorTotal:string,
  lowerMotorTotal:string,
  touchTotal:string,
  prickTotal:string,
  rightSensory:string,
  leftSensory:string,
  rightMotor:string,
  leftMotor:string,
  neurologicalLevelOfInjury:string,
  rightSensoryZpp:string,
  leftSensoryZpp:string,
  rightMotorZpp:string,
  leftMotorZpp:string,
  asiaImpairmentScale:string
}

export interface iValidationTestModel {
  id:number,
  group:string,
  examDate:string,
  analContraction:string,
  analSensation:string,
  rightLowestNonKeyMuscleWithMotorFunction:string,
  leftLowestNonKeyMuscleWithMotorFunction:string,
  comments:string,
  dermatomes: iDermatomesModel,
  totals:iTotalsModel
}

export var rhiIsncsciValidationTests:iValidationTestModel[] = [
  {
    id: 1,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Assesment Case A",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "4"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "4"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "13",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "12",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "16",
      rightTouchContainsNt: false,
      leftTouchTotal: "10",
      leftTouchContainsNt: false,
      rightPrickTotal: "11",
      rightPrickContainsNt: false,
      leftPrickTotal: "10",
      leftPrickContainsNt: false,
      rightMotorTotal: "13",
      leftMotorTotal: "12",
      upperMotorTotal: "25",
      lowerMotorTotal: "0",
      touchTotal: "26",
      prickTotal: "21",
      rightSensory: "C6",
      leftSensory: "C6",
      rightMotor: "C6",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "T5",
      leftSensoryZpp: "C6",
      rightMotorZpp: "T1",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 2,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Assesment Case B",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "37",
      leftTouchContainsNt: false,
      rightPrickTotal: "38",
      rightPrickContainsNt: false,
      leftPrickTotal: "36",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "25",
      upperMotorTotal: "50",
      lowerMotorTotal: "0",
      touchTotal: "75",
      prickTotal: "74",
      rightSensory: "T12",
      leftSensory: "T11",
      rightMotor: "T12",
      leftMotor: "T11",
      neurologicalLevelOfInjury: "T11",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "T12",
      rightMotorZpp: "T12",
      leftMotorZpp: "T11",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 3,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Assesment Case C",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "4"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "3",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "4",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "43",
      rightTouchContainsNt: false,
      leftTouchTotal: "44",
      leftTouchContainsNt: false,
      rightPrickTotal: "42",
      rightPrickContainsNt: false,
      leftPrickTotal: "45",
      leftPrickContainsNt: false,
      rightMotorTotal: "28",
      leftMotorTotal: "29",
      upperMotorTotal: "50",
      lowerMotorTotal: "7",
      touchTotal: "87",
      prickTotal: "87",
      rightSensory: "T12",
      leftSensory: "L1",
      rightMotor: "T12",
      leftMotor: "L2",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "L2",
      leftMotorZpp: "L2",
      asiaImpairmentScale: "B"
    }
  },
  {
    id: 4,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Assesment Case D",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "4",
        leftMotor: "4"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "3"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "2",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "4"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "3"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "4"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "4"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "4"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "9",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "10",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "10",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "19",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "17",
      rightTouchContainsNt: false,
      leftTouchTotal: "19",
      leftTouchContainsNt: false,
      rightPrickTotal: "8",
      rightPrickContainsNt: false,
      leftPrickTotal: "12",
      leftPrickContainsNt: false,
      rightMotorTotal: "19",
      leftMotorTotal: "29",
      upperMotorTotal: "19",
      lowerMotorTotal: "29",
      touchTotal: "36",
      prickTotal: "20",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C5",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 5,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Assesment Case E",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "1",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "4",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "40",
      rightTouchContainsNt: false,
      leftTouchTotal: "40",
      leftTouchContainsNt: false,
      rightPrickTotal: "40",
      rightPrickContainsNt: false,
      leftPrickTotal: "40",
      leftPrickContainsNt: false,
      rightMotorTotal: "26",
      leftMotorTotal: "29",
      upperMotorTotal: "50",
      lowerMotorTotal: "5",
      touchTotal: "80",
      prickTotal: "80",
      rightSensory: "T12",
      leftSensory: "T12",
      rightMotor: "T12",
      leftMotor: "T12",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "L2",
      leftSensoryZpp: "L2",
      rightMotorZpp: "L2",
      leftMotorZpp: "L3",
      asiaImpairmentScale: "B"
    }
  },
  {
    id: 6,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Sample Case 1",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "10",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "10",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "10",
      rightTouchContainsNt: false,
      leftTouchTotal: "10",
      leftTouchContainsNt: false,
      rightPrickTotal: "10",
      rightPrickContainsNt: false,
      leftPrickTotal: "10",
      leftPrickContainsNt: false,
      rightMotorTotal: "10",
      leftMotorTotal: "10",
      upperMotorTotal: "20",
      lowerMotorTotal: "0",
      touchTotal: "20",
      prickTotal: "20",
      rightSensory: "C6",
      leftSensory: "C6",
      rightMotor: "C6",
      leftMotor: "C6",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "C6",
      leftSensoryZpp: "C6",
      rightMotorZpp: "C7",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 7,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Sample Case 2",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "4"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "2"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "4",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "6",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "7",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "7",
      leftPrickContainsNt: false,
      rightMotorTotal: "4",
      leftMotorTotal: "6",
      upperMotorTotal: "10",
      lowerMotorTotal: "0",
      touchTotal: "13",
      prickTotal: "13",
      rightSensory: "C3",
      leftSensory: "C4",
      rightMotor: "C3",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C3",
      rightSensoryZpp: "C5",
      leftSensoryZpp: "C5",
      rightMotorZpp: "C6",
      leftMotorZpp: "C6",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 8,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Sample Case 3",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "2"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "4",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "42",
      leftTouchContainsNt: false,
      rightPrickTotal: "38",
      rightPrickContainsNt: false,
      leftPrickTotal: "38",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "29",
      upperMotorTotal: "50",
      lowerMotorTotal: "4",
      touchTotal: "80",
      prickTotal: "76",
      rightSensory: "T12",
      leftSensory: "T12",
      rightMotor: "T12",
      leftMotor: "T12",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "L4",
      rightMotorZpp: "T12",
      leftMotorZpp: "L4",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 9,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Sample Case 4",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "2"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "2",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "42",
      leftTouchContainsNt: false,
      rightPrickTotal: "38",
      rightPrickContainsNt: false,
      leftPrickTotal: "38",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "27",
      upperMotorTotal: "50",
      lowerMotorTotal: "2",
      touchTotal: "80",
      prickTotal: "76",
      rightSensory: "T12",
      leftSensory: "T12",
      rightMotor: "T12",
      leftMotor: "T12",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "L4",
      rightMotorZpp: "T12",
      leftMotorZpp: "L2",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 10,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Sample Case 5",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "3"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "0",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "3"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "14",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "11",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "14",
      rightTouchContainsNt: false,
      leftTouchTotal: "11",
      leftTouchContainsNt: false,
      rightPrickTotal: "14",
      rightPrickContainsNt: false,
      leftPrickTotal: "11",
      leftPrickContainsNt: false,
      rightMotorTotal: "14",
      leftMotorTotal: "11",
      upperMotorTotal: "25",
      lowerMotorTotal: "0",
      touchTotal: "25",
      prickTotal: "25",
      rightSensory: "C7",
      leftSensory: "C6",
      rightMotor: "C7",
      leftMotor: "C6",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "C7",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 11,
    group: "Instep",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "InStep - Module 5 - Sample Case 6",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "2",
        leftMotor: "4"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "1"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "16",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "24",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "16",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "21",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "32",
      rightTouchContainsNt: false,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "32",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "32",
      leftMotorTotal: "45",
      upperMotorTotal: "40",
      lowerMotorTotal: "37",
      touchTotal: "88",
      prickTotal: "88",
      rightSensory: "C5",
      leftSensory: "S4_5",
      rightMotor: "C6",
      leftMotor: "T1",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 12,
    group: "AsiaSpecialArticle",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "NT - Severe left wrist pain.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "NT"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "2"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "13",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "10",
      rightTouchContainsNt: false,
      leftTouchTotal: "10",
      leftTouchContainsNt: false,
      rightPrickTotal: "10",
      rightPrickContainsNt: false,
      leftPrickTotal: "10",
      leftPrickContainsNt: false,
      rightMotorTotal: "13",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "20",
      prickTotal: "20",
      rightSensory: "C6",
      leftSensory: "C6",
      rightMotor: "C7",
      leftMotor: "C5,C6",
      neurologicalLevelOfInjury: "C5,C6",
      rightSensoryZpp: "C6",
      leftSensoryZpp: "C6",
      rightMotorZpp: "C7",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 13,
    group: "AsiaSpecialArticle",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "NT- Splint in place on left hand unable to test T1 motor.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "2"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "2"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "2"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "8",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "32",
      rightTouchContainsNt: false,
      leftTouchTotal: "32",
      leftTouchContainsNt: false,
      rightPrickTotal: "32",
      rightPrickContainsNt: false,
      leftPrickTotal: "32",
      leftPrickContainsNt: false,
      rightMotorTotal: "8",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "64",
      prickTotal: "64",
      rightSensory: "C5",
      leftSensory: "C5",
      rightMotor: "C5",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "C8",
      leftMotorZpp: "C8,T1",
      asiaImpairmentScale: "B,C"
    }
  },
  {
    id: 14,
    group: "AsiaSpecialArticle",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "NT - Splint in place on left hand and unable to test C8 and T1 motor.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "3"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "NT"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "1"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "3"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "2"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "2"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "21",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "11",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "9",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "34",
      rightTouchContainsNt: false,
      leftTouchTotal: "34",
      leftTouchContainsNt: false,
      rightPrickTotal: "34",
      rightPrickContainsNt: false,
      leftPrickTotal: "34",
      leftPrickContainsNt: false,
      rightMotorTotal: "32",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "20",
      touchTotal: "68",
      prickTotal: "68",
      rightSensory: "C7",
      leftSensory: "C7",
      rightMotor: "C8",
      leftMotor: "C7",
      neurologicalLevelOfInjury: "C7",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "C,D"
    }
  },
  {
    id: 15,
    group: "AsiaSpecialArticle",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Note - Radial nerve palsy on the left side affecting the left C6 motor and left C6 and C7 sensory testing.\n ~C6 to C7 Left Touch:\n ~C6 to C7 Left Prick:\n ~C6 Left Motor:",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "1!",
        rightPrick: "2",
        leftPrick: "0!",
        rightMotor: "5",
        leftMotor: "0!"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "1!",
        rightPrick: "2",
        leftPrick: "0!",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "20!",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "26",
      rightTouchContainsNt: false,
      leftTouchTotal: "24!",
      leftTouchContainsNt: false,
      rightPrickTotal: "26",
      rightPrickContainsNt: false,
      leftPrickTotal: "22!",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "20!",
      upperMotorTotal: "45!",
      lowerMotorTotal: "0",
      touchTotal: "50!",
      prickTotal: "48!",
      rightSensory: "T6",
      leftSensory: "T6",
      rightMotor: "T6",
      leftMotor: "T6",
      neurologicalLevelOfInjury: "T6",
      rightSensoryZpp: "T6",
      leftSensoryZpp: "T6",
      rightMotorZpp: "T6",
      leftMotorZpp: "T6",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 16,
    group: "AsiaSpecialArticle",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "4",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "2"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "13",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "12",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "8",
      rightTouchContainsNt: false,
      leftTouchTotal: "8",
      leftTouchContainsNt: false,
      rightPrickTotal: "8",
      rightPrickContainsNt: false,
      leftPrickTotal: "8",
      leftPrickContainsNt: false,
      rightMotorTotal: "13",
      leftMotorTotal: "12",
      upperMotorTotal: "25",
      lowerMotorTotal: "0",
      touchTotal: "16",
      prickTotal: "16",
      rightSensory: "C5",
      leftSensory: "C5",
      rightMotor: "C6",
      leftMotor: "C6",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "C5",
      leftSensoryZpp: "C5",
      rightMotorZpp: "T1",
      leftMotorZpp: "C8",
      asiaImpairmentScale: "B"
    }
  },
  {
    id: 17,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case - Bug discovered by Dr. Reeves - Feb 9th, 2012",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "3"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "3"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "10",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "8",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "9",
      rightTouchContainsNt: false,
      leftTouchTotal: "9",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "8",
      leftPrickContainsNt: false,
      rightMotorTotal: "10",
      leftMotorTotal: "8",
      upperMotorTotal: "18",
      lowerMotorTotal: "0",
      touchTotal: "18",
      prickTotal: "14",
      rightSensory: "C3",
      leftSensory: "C4",
      rightMotor: "C3",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C3",
      rightSensoryZpp: "C7",
      leftSensoryZpp: "C7",
      rightMotorZpp: "C7",
      leftMotorZpp: "C8",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 18,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "A 5* on a motor cell does not rise the flags: RightUpperMotorTotalHasImpairmentNotDueToSci, LeftUpperMotorTotalHasImpairmentNotDueToSci, RightLowerMotorTotalHasImpairmentNotDueToSci or LeftLowerMotorTotalHasImpairmentNotDueToSci.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "56",
      rightTouchContainsNt: false,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "112",
      prickTotal: "112",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 19,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "There was an issue when the single level is less than C5 and there are no motor scores. The most caudal right motor property was null, since no motor value was forcing it to update. This caused a null reference exception. As fix, before calculating the totals, we check if the property is null. We set it to equal the value for right sensory if so.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "2",
      rightTouchContainsNt: false,
      leftTouchTotal: "2",
      leftTouchContainsNt: false,
      rightPrickTotal: "2",
      rightPrickContainsNt: false,
      leftPrickTotal: "2",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "4",
      prickTotal: "4",
      rightSensory: "C2",
      leftSensory: "C2",
      rightMotor: "C2",
      leftMotor: "C2",
      neurologicalLevelOfInjury: "C2",
      rightSensoryZpp: "C2",
      leftSensoryZpp: "C2",
      rightMotorZpp: "C2",
      leftMotorZpp: "C2",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 20,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "There was an issue when the single level is less than C5 and there are no motor scores. The most caudal left motor property was null, since no motor value was forcing it to update. This caused a null reference exception. As fix, before calculating the totals, we check if the property is null. We set it to equal the value for left sensory if so.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "5",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "2",
      rightTouchContainsNt: false,
      leftTouchTotal: "2",
      leftTouchContainsNt: false,
      rightPrickTotal: "2",
      rightPrickContainsNt: false,
      leftPrickTotal: "2",
      leftPrickContainsNt: false,
      rightMotorTotal: "5",
      leftMotorTotal: "0",
      upperMotorTotal: "5",
      lowerMotorTotal: "0",
      touchTotal: "4",
      prickTotal: "4",
      rightSensory: "C2",
      leftSensory: "C2",
      rightMotor: "C2",
      leftMotor: "C2",
      neurologicalLevelOfInjury: "C2",
      rightSensoryZpp: "C2",
      leftSensoryZpp: "C2",
      rightMotorZpp: "T1",
      leftMotorZpp: "C2",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 21,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case - Bug discovered by Dr. Reeves - Feb 9th, 2012",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      C4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      C5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "4"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "3"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "10",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "18",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "4",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "12",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "26",
      rightTouchContainsNt: false,
      leftTouchTotal: "26",
      leftTouchContainsNt: false,
      rightPrickTotal: "26",
      rightPrickContainsNt: false,
      leftPrickTotal: "26",
      leftPrickContainsNt: false,
      rightMotorTotal: "14",
      leftMotorTotal: "30",
      upperMotorTotal: "28",
      lowerMotorTotal: "16",
      touchTotal: "52",
      prickTotal: "52",
      rightSensory: "C2",
      leftSensory: "C2",
      rightMotor: "C2",
      leftMotor: "C2",
      neurologicalLevelOfInjury: "C2",
      rightSensoryZpp: "S1",
      leftSensoryZpp: "S1",
      rightMotorZpp: "L5",
      leftMotorZpp: "L5",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 22,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case - Bug discovered by Dr. Reeves - Feb 9th, 2012",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "10",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "10",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "8",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "8",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "28",
      rightTouchContainsNt: false,
      leftTouchTotal: "28",
      leftTouchContainsNt: false,
      rightPrickTotal: "29",
      rightPrickContainsNt: false,
      leftPrickTotal: "29",
      leftPrickContainsNt: false,
      rightMotorTotal: "18",
      leftMotorTotal: "18",
      upperMotorTotal: "20",
      lowerMotorTotal: "16",
      touchTotal: "56",
      prickTotal: "58",
      rightSensory: "C3",
      leftSensory: "C3",
      rightMotor: "C3",
      leftMotor: "C3",
      neurologicalLevelOfInjury: "C3",
      rightSensoryZpp: "S2",
      leftSensoryZpp: "S2",
      rightMotorZpp: "L5",
      leftMotorZpp: "L5",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 23,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "T2",
    leftLowestNonKeyMuscleWithMotorFunction: "T5",
    comments: "Edge case - Bug discovered by Dr. Waring - Feb 9th, 2012, regarding non-key muscles.\n [EE: Aug 20, 2012] This case got ammended after implementing Ronald Reeves suggestion on reflecting the non-key muscles on the ZPP.\n Before the changes the Most Caudal Right Motor Name was C5 and the Most Caudal Left Motor Name was C6.\n After the changes the Most Caudal Right Motor Name became T2 and the Most Caudal Left Motor Name became T5\n Right motor function at T2\n Left motor function at T5",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "0",
        leftMotor: "3"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "5",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "8",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "33",
      rightTouchContainsNt: false,
      leftTouchTotal: "33",
      leftTouchContainsNt: false,
      rightPrickTotal: "33",
      rightPrickContainsNt: false,
      leftPrickTotal: "33",
      leftPrickContainsNt: false,
      rightMotorTotal: "5",
      leftMotorTotal: "8",
      upperMotorTotal: "13",
      lowerMotorTotal: "0",
      touchTotal: "66",
      prickTotal: "66",
      rightSensory: "C6",
      leftSensory: "C6",
      rightMotor: "C5",
      leftMotor: "C6",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "T2",
      leftMotorZpp: "T5",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 24,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Defect found after our first public release.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "2"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "4"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "4"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "4"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "24",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "18",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "24",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "56",
      rightTouchContainsNt: false,
      leftTouchTotal: "54",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "54",
      leftPrickContainsNt: false,
      rightMotorTotal: "48",
      leftMotorTotal: "43",
      upperMotorTotal: "42",
      lowerMotorTotal: "49",
      touchTotal: "110",
      prickTotal: "110",
      rightSensory: "S4_5",
      leftSensory: "C6",
      rightMotor: "C5",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 25,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Defect found after our first public release.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "0",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "6",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "17",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "48",
      rightTouchContainsNt: false,
      leftTouchTotal: "39",
      leftTouchContainsNt: false,
      rightPrickTotal: "42",
      rightPrickContainsNt: false,
      leftPrickTotal: "37",
      leftPrickContainsNt: false,
      rightMotorTotal: "23",
      leftMotorTotal: "0",
      upperMotorTotal: "6",
      lowerMotorTotal: "17",
      touchTotal: "87",
      prickTotal: "79",
      rightSensory: "C6",
      leftSensory: "C5",
      rightMotor: "C5",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 26,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Defect found after our first public release.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "15",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "11",
      rightTouchContainsNt: false,
      leftTouchTotal: "9",
      leftTouchContainsNt: false,
      rightPrickTotal: "10",
      rightPrickContainsNt: false,
      leftPrickTotal: "9",
      leftPrickContainsNt: false,
      rightMotorTotal: "15",
      leftMotorTotal: "0",
      upperMotorTotal: "15",
      lowerMotorTotal: "0",
      touchTotal: "20",
      prickTotal: "19",
      rightSensory: "C5",
      leftSensory: "C5",
      rightMotor: "C5",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "T1",
      leftSensoryZpp: "C8",
      rightMotorZpp: "T1",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 27,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Error calculating motor scores when sensory is not 2 on a line with all motor scores set to 5",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "54",
      rightTouchContainsNt: false,
      leftTouchTotal: "54",
      leftTouchContainsNt: false,
      rightPrickTotal: "54",
      rightPrickContainsNt: false,
      leftPrickTotal: "54",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "108",
      prickTotal: "108",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "T1",
      leftMotor: "T1",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 28,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Impairment not due to SCI should not be reflected on the motor totals when a sensory value has it on a non-key muscle. This is a check for the algorithm sice I know how it is performing the comparisons.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "0!",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "54!",
      rightTouchContainsNt: false,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "110!",
      prickTotal: "112",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 29,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Motor ZPP is incorrect when all motor values are set to 0 and there are normal sensory values in the first four rows.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "8",
      rightTouchContainsNt: false,
      leftTouchTotal: "8",
      leftTouchContainsNt: false,
      rightPrickTotal: "8",
      rightPrickContainsNt: false,
      leftPrickTotal: "8",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "16",
      prickTotal: "16",
      rightSensory: "C5",
      leftSensory: "C5",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C5",
      leftSensoryZpp: "C5",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 30,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Motor ZPP is incorrect when all motor values are set to 0 and there are normal sensory values in the first four rows.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "0"
      },
      C3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      C4: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "4",
      rightTouchContainsNt: false,
      leftTouchTotal: "3",
      leftTouchContainsNt: false,
      rightPrickTotal: "2",
      rightPrickContainsNt: false,
      leftPrickTotal: "0",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "7",
      prickTotal: "2",
      rightSensory: "C2",
      leftSensory: "C1",
      rightMotor: "C2",
      leftMotor: "C1",
      neurologicalLevelOfInjury: "C1",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C3",
      rightMotorZpp: "C2",
      leftMotorZpp: "C1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 31,
    group: "Fixes",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ZPP for motor does not seem to be calculating correctly. It is giving a value of S4-5 for motor when it should be S1.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "43",
      leftTouchContainsNt: false,
      rightPrickTotal: "40",
      rightPrickContainsNt: false,
      leftPrickTotal: "40",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "25",
      touchTotal: "81",
      prickTotal: "80",
      rightSensory: "T12",
      leftSensory: "L1",
      rightMotor: "T12",
      leftMotor: "S1",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "L1",
      leftSensoryZpp: "L4",
      rightMotorZpp: "T12",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 32,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 1",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4,T1",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 33,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 2\n NT 2 degree ++ spasms and pain",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "6",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "6",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "12",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C5",
      leftSensory: "C5",
      rightMotor: "C5",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "C6,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "C6,L2,L3,L4,L5,S1",
      leftMotorZpp: "C6,L2,L3,L4,L5,S1",
      asiaImpairmentScale: "B,C,D"
    }
  },
  {
    id: 34,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 3\n Cast (L) wrist\/hand",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "NT"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "NT"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "26",
      rightTouchContainsNt: false,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "26",
      rightPrickContainsNt: false,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "25",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "T6",
      leftSensory: "C5,C6,C7,T6",
      rightMotor: "T6",
      leftMotor: "C5,C6,C7,C8,T1,T6",
      neurologicalLevelOfInjury: "C5,C6,C7,C8,T1,T6",
      rightSensoryZpp: "T6",
      leftSensoryZpp: "T6",
      rightMotorZpp: "T6",
      leftMotorZpp: "C7,C8,T1,T6",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 35,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "NT",
    analSensation: "NT",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 4\n Previous abdominoperitoneal surgery (no rectum) & skin grafts on bottom.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "2"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "1"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "3"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "3"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "4"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "3"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "3"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "10",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "10",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "16",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "16",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "26",
      leftMotorTotal: "26",
      upperMotorTotal: "20",
      lowerMotorTotal: "32",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C5",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "S2,S3",
      leftSensoryZpp: "S2,S3",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "A,D"
    }
  },
  {
    id: 36,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "12",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "11",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "32",
      rightTouchContainsNt: false,
      leftTouchTotal: "32",
      leftTouchContainsNt: false,
      rightPrickTotal: "32",
      rightPrickContainsNt: false,
      leftPrickTotal: "32",
      leftPrickContainsNt: false,
      rightMotorTotal: "UTD",
      leftMotorTotal: "11",
      upperMotorTotal: "23",
      lowerMotorTotal: "UTD",
      touchTotal: "64",
      prickTotal: "64",
      rightSensory: "C6",
      leftSensory: "C6",
      rightMotor: "C6",
      leftMotor: "C6",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "C8,L5",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "B,C"
    }
  },
  {
    id: 37,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "0",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "2",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "12",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "34",
      leftTouchContainsNt: false,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "34",
      leftPrickContainsNt: false,
      rightMotorTotal: "UTD",
      leftMotorTotal: "50",
      upperMotorTotal: "37",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "L1,L2,S4_5",
      leftSensory: "C6",
      rightMotor: "C6",
      leftMotor: "T1",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 38,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "UTD",
      prickTotal: "112",
      rightSensory: "C4,C5,C6,L1,S4_5",
      leftSensory: "S4_5",
      rightMotor: "T1,S1,S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "C4,C5,C6,L1,S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "",
      asiaImpairmentScale: "D,E"
    }
  },
  {
    id: 39,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "UTD",
      prickTotal: "112",
      rightSensory: "C4,S4_5",
      leftSensory: "S4_5",
      rightMotor: "T1,S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "C4,S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "",
      asiaImpairmentScale: "D,E"
    }
  },
  {
    id: 40,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "NT",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "UTD",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "UTD",
      touchTotal: "12",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C4,S1",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 41,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "UTD",
      upperMotorTotal: "0",
      lowerMotorTotal: "UTD",
      touchTotal: "12",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4,S1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 42,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "NT",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "12",
      prickTotal: "UTD",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4,T1",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 43,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "12",
      prickTotal: "UTD",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4,T1",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 44,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "NT",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4,T1",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 45,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "NT",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "UTD",
      rightUpperMotorContainsNt: true,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "UTD",
      leftMotorTotal: "0",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "12",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C4,T1",
      leftMotorZpp: "C4",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 46,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "12",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C4",
      leftMotorZpp: "C4,T1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 47,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "NT",
    analSensation: "NT",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      C3: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      C4: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      C5: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      C6: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      C7: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      C8: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      T1: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      T2: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T3: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T4: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T5: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T6: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T7: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T8: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T9: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T10: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T11: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      T12: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      L1: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      L2: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      L3: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      L4: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      L5: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      S1: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!",
        rightMotor: "NT!",
        leftMotor: "NT!"
      },
      S2: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      S3: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      },
      S4_5: {
        rightTouch: "NT!",
        leftTouch: "NT!",
        rightPrick: "NT!",
        leftPrick: "NT!"
      }
    },
    totals: {
      rightUpperMotorTotal: "0!",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0!",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0!",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0!",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "0!",
      rightTouchContainsNt: false,
      leftTouchTotal: "0!",
      leftTouchContainsNt: false,
      rightPrickTotal: "0!",
      rightPrickContainsNt: false,
      leftPrickTotal: "0!",
      leftPrickContainsNt: false,
      rightMotorTotal: "0!",
      leftMotorTotal: "0!",
      upperMotorTotal: "0!",
      lowerMotorTotal: "0!",
      touchTotal: "0!",
      prickTotal: "0!",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 48,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "2"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "1",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "3",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "2",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "31",
      rightPrickContainsNt: false,
      leftPrickTotal: "31",
      leftPrickContainsNt: false,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "5",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "62",
      rightSensory: "C1,C2,C3,C4",
      leftSensory: "C1,C2,C3,C4",
      rightMotor: "C1,C2,C3,C5",
      leftMotor: "C1,C2,C3,C4",
      neurologicalLevelOfInjury: "C1,C2,C3,C4",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "C5,L2,L3,L4,L5,S1",
      leftMotorZpp: "C5,L2,L3,L4,L5,S1",
      asiaImpairmentScale: "C,D"
    }
  },
  {
    id: 49,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "25",
      leftMotorTotal: "25",
      upperMotorTotal: "50",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "T10",
      leftSensory: "T10",
      rightMotor: "T10",
      leftMotor: "T10",
      neurologicalLevelOfInjury: "T10",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "T12",
      rightMotorZpp: "T10",
      leftMotorZpp: "T10",
      asiaImpairmentScale: "A,B"
    }
  },
  {
    id: 50,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "NT",
    analSensation: "NT",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "25",
      leftMotorTotal: "25",
      upperMotorTotal: "50",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "T10",
      leftSensory: "T10",
      rightMotor: "T10",
      leftMotor: "T10",
      neurologicalLevelOfInjury: "T10",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "T12",
      rightMotorZpp: "T10",
      leftMotorZpp: "T10",
      asiaImpairmentScale: "A,B,C"
    }
  },
  {
    id: 51,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "NT",
    analSensation: "NT",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "S3,S4_5",
      leftSensory: "S3,S4_5",
      rightMotor: "S3,S4_5",
      leftMotor: "S3,S4_5",
      neurologicalLevelOfInjury: "S3,S4_5",
      rightSensoryZpp: "S3",
      leftSensoryZpp: "S3",
      rightMotorZpp: "S3",
      leftMotorZpp: "S3",
      asiaImpairmentScale: "A,B,D,E"
    }
  },
  {
    id: 52,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "NT",
    analSensation: "NT",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 21:\n The algorithm was not running all possibilities for when VAC and DAP have NT.\n The algorithm was reporting an AIS score equal to A and D, when it should be A, B, C, and D.",
    dermatomes: {
      C2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "4",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "4"
      },
      C8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "2"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "2"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "9",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "17",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "26",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightMotor: "C1,C2,C3,C6",
      leftMotor: "C1,C2,C3,C6",
      neurologicalLevelOfInjury: "C1,C2,C3,C4,C5,C6",
      rightSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "C6,L2,L3,L4,L5,S1",
      leftMotorZpp: "T1,L2,L3,L4,L5,S1",
      asiaImpairmentScale: "A,B,C,D"
    }
  },
  {
    id: 53,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 22:\n This case is being reviewed to determine if a form will all intact and no for VAC and\/or DAP should be E or something else.",
    dermatomes: {
      C2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "50",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightMotor: "C1,C2,C3,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftMotor: "C1,C2,C3,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      neurologicalLevelOfInjury: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftMotorZpp: "T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      asiaImpairmentScale: "B,C,D,E"
    }
  },
  {
    id: 54,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "NT",
    analSensation: "NT",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "AIS D should also be reported as a possibility. Currently only B, C and E are.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "UTD",
      rightUpperMotorContainsNt: true,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftSensory: "S4_5",
      rightMotor: "C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftMotor: "C4,C5,C6,C7,C8,T1,L1,L2,L3,L4,L5,S1,S4_5",
      neurologicalLevelOfInjury: "C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightSensoryZpp: "C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "",
      rightMotorZpp: "C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftMotorZpp: "C4,C5,C6,C7,C8,T1,L1,L2,L3,L4,L5,S1",
      asiaImpairmentScale: "B,C,D,E"
    }
  },
  {
    id: 55,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 24\n AIS D should also be reported as a possibility. Currently only A and B are.",
    dermatomes: {
      C2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "4",
        leftMotor: "4"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "24",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "24",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "24",
      leftMotorTotal: "24",
      upperMotorTotal: "48",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightMotor: "C1,C2,C3,T1",
      leftMotor: "C1,C2,C3,T1",
      neurologicalLevelOfInjury: "C1,C2,C3,C4,C5,C6,C7,C8,T1",
      rightSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "T1",
      leftMotorZpp: "T1",
      asiaImpairmentScale: "A,B,D"
    }
  },
  {
    id: 56,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 25\n AIS B should also be reported as a possibility. Currently only C is.",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "50",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "T12",
      leftSensory: "T12",
      rightMotor: "T12",
      leftMotor: "T12",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "S3",
      leftSensoryZpp: "S3",
      rightMotorZpp: "T12,L4,S1",
      leftMotorZpp: "T12,L4,S1",
      asiaImpairmentScale: "B,C"
    }
  },
  {
    id: 57,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 26",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "3"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "19",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "21",
      rightTouchContainsNt: false,
      leftTouchTotal: "17",
      leftTouchContainsNt: false,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "25",
      leftMotorTotal: "19",
      upperMotorTotal: "44",
      lowerMotorTotal: "0",
      touchTotal: "38",
      prickTotal: "UTD",
      rightSensory: "C6",
      leftSensory: "C5",
      rightMotor: "T1",
      leftMotor: "C7",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "T4",
      leftSensoryZpp: "T4",
      rightMotorZpp: "T1",
      leftMotorZpp: "T1",
      asiaImpairmentScale: "A,B"
    }
  },
  {
    id: 58,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 24\n AIS D should also be reported as a possibility. Currently only A and B are.",
    dermatomes: {
      C2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      C8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "UTD",
      rightUpperMotorContainsNt: true,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightMotor: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1",
      leftMotor: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1",
      neurologicalLevelOfInjury: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1",
      rightSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1",
      leftMotorZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1",
      asiaImpairmentScale: "B,C,D"
    }
  },
  {
    id: 59,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 28",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "NT",
        rightPrick: "2",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "50",
      lowerMotorTotal: "UTD",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "L4,L5,S1,S2,S3,S4_5",
      leftSensory: "L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightMotor: "L1,L2,L3",
      leftMotor: "L1,L2",
      neurologicalLevelOfInjury: "L1,L2",
      rightSensoryZpp: "L4,L5,S1,S2,S3",
      leftSensoryZpp: "L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "L3",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "C,D"
    }
  },
  {
    id: 60,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "ISNCSCI NT Case 28",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "4"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "NT",
        leftMotor: "NT"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "UTD",
      rightLowerMotorContainsNt: true,
      leftLowerMotorTotal: "UTD",
      leftLowerMotorContainsNt: true,
      rightTouchTotal: "53",
      rightTouchContainsNt: false,
      leftTouchTotal: "53",
      leftTouchContainsNt: false,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "UTD",
      leftMotorTotal: "UTD",
      upperMotorTotal: "50",
      lowerMotorTotal: "UTD",
      touchTotal: "106",
      prickTotal: "UTD",
      rightSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3",
      leftSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3",
      rightMotor: "C1,C2,C3,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1",
      leftMotor: "C1,C2,C3,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3",
      neurologicalLevelOfInjury: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "L3,L4,L5,S1",
      leftMotorZpp: "L3,L4,L5,S1",
      asiaImpairmentScale: "B,C,D"
    }
  },
  {
    id: 61,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      C5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "4",
        leftMotor: "NT"
      },
      C7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "4",
        leftMotor: "4"
      },
      C8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "4",
        leftMotor: "3"
      },
      T1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "2",
        leftMotor: "NT"
      },
      T2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T6: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T7: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T8: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T9: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T10: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T11: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      T12: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      L2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S3: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      },
      S4_5: {
        rightTouch: "NT",
        leftTouch: "NT",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "19",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "UTD",
      leftUpperMotorContainsNt: true,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "UTD",
      rightTouchContainsNt: true,
      leftTouchTotal: "UTD",
      leftTouchContainsNt: true,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "19",
      leftMotorTotal: "UTD",
      upperMotorTotal: "UTD",
      lowerMotorTotal: "0",
      touchTotal: "UTD",
      prickTotal: "UTD",
      rightSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      leftSensory: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3,S4_5",
      rightMotor: "C1,C2,C3,C6",
      leftMotor: "C1,C2,C3,C5,C6,C7",
      neurologicalLevelOfInjury: "C1,C2,C3,C4,C5,C6",
      rightSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      leftSensoryZpp: "C1,C2,C3,C4,C5,C6,C7,C8,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,L1,L2,L3,L4,L5,S1,S2,S3",
      rightMotorZpp: "T1",
      leftMotorZpp: "C8,T1",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 62,
    group: "NtCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "3"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "NT",
        leftPrick: "NT"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "19",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "21",
      rightTouchContainsNt: false,
      leftTouchTotal: "17",
      leftTouchContainsNt: false,
      rightPrickTotal: "UTD",
      rightPrickContainsNt: true,
      leftPrickTotal: "UTD",
      leftPrickContainsNt: true,
      rightMotorTotal: "25",
      leftMotorTotal: "19",
      upperMotorTotal: "44",
      lowerMotorTotal: "0",
      touchTotal: "38",
      prickTotal: "UTD",
      rightSensory: "C6",
      leftSensory: "C5",
      rightMotor: "T1",
      leftMotor: "C7",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "T4",
      leftSensoryZpp: "T4",
      rightMotorZpp: "T1",
      leftMotorZpp: "T1",
      asiaImpairmentScale: "A,B"
    }
  },
  {
    id: 63,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "",
    leftLowestNonKeyMuscleWithMotorFunction: "",
    comments: "ASIA E with 5* values",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5*",
        leftMotor: "5*"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "56",
      rightTouchContainsNt: false,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "112",
      prickTotal: "112",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 64,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Given that I complete a form with all values set to Zero, I get an ASIA impairment of C1",
    dermatomes: {
      C2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "0",
      rightTouchContainsNt: false,
      leftTouchTotal: "0",
      leftTouchContainsNt: false,
      rightPrickTotal: "0",
      rightPrickContainsNt: false,
      leftPrickTotal: "0",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "0",
      prickTotal: "0",
      rightSensory: "C1",
      leftSensory: "C1",
      rightMotor: "C1",
      leftMotor: "C1",
      neurologicalLevelOfInjury: "C1",
      rightSensoryZpp: "C1",
      leftSensoryZpp: "C1",
      rightMotorZpp: "C1",
      leftMotorZpp: "C1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 65,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Given that I complete a form with impairment values not due to SCI, I get an ASIA impairment score equal to E",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "1!",
        leftTouch: "1!",
        rightPrick: "1!",
        leftPrick: "1!"
      },
      C4: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      C5: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "4!",
        leftMotor: "4!"
      },
      C7: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "3!",
        leftMotor: "3!"
      },
      C8: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "2!",
        leftMotor: "2!"
      },
      T1: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "1!",
        leftMotor: "1!"
      },
      T2: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T3: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T4: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T5: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T6: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T7: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T8: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T9: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T10: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T11: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      T12: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      L1: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      L2: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "0!",
        leftMotor: "0!"
      },
      L3: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "0!",
        leftMotor: "0!"
      },
      L4: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "0!",
        leftMotor: "0!"
      },
      L5: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "0!",
        leftMotor: "0!"
      },
      S1: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!",
        rightMotor: "0!",
        leftMotor: "0!"
      },
      S2: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      S3: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      S4_5: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      }
    },
    totals: {
      rightUpperMotorTotal: "15!",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "15!",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0!",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0!",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "3!",
      rightTouchContainsNt: false,
      leftTouchTotal: "3!",
      leftTouchContainsNt: false,
      rightPrickTotal: "3!",
      rightPrickContainsNt: false,
      leftPrickTotal: "3!",
      leftPrickContainsNt: false,
      rightMotorTotal: "15!",
      leftMotorTotal: "15!",
      upperMotorTotal: "30!",
      lowerMotorTotal: "0!",
      touchTotal: "6!",
      prickTotal: "6!",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 66,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "T4",
    leftLowestNonKeyMuscleWithMotorFunction: "",
    comments: "Edge case 1. [EE: Aug 20, 2012] This case got ammended after implementing Ronald Reeves suggestion on reflecting the non-key muscles on the ZPP.\n Before the changes the Most Caudal Right Motor Name was C7.\n After the changes the Most Caudal Right Motor Name became T4\n Right motor function at T4",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "0",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "3"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "14",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "13",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "14",
      rightTouchContainsNt: false,
      leftTouchTotal: "11",
      leftTouchContainsNt: false,
      rightPrickTotal: "14",
      rightPrickContainsNt: false,
      leftPrickTotal: "11",
      leftPrickContainsNt: false,
      rightMotorTotal: "14",
      leftMotorTotal: "13",
      upperMotorTotal: "27",
      lowerMotorTotal: "0",
      touchTotal: "25",
      prickTotal: "25",
      rightSensory: "C7",
      leftSensory: "C6",
      rightMotor: "C7",
      leftMotor: "C7",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "T4",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 67,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 2. Given that I enter all the values for case 13, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "37",
      leftTouchContainsNt: false,
      rightPrickTotal: "38",
      rightPrickContainsNt: false,
      leftPrickTotal: "36",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "25",
      upperMotorTotal: "50",
      lowerMotorTotal: "0",
      touchTotal: "75",
      prickTotal: "74",
      rightSensory: "T12",
      leftSensory: "T11",
      rightMotor: "T12",
      leftMotor: "T11",
      neurologicalLevelOfInjury: "T11",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "T12",
      rightMotorZpp: "T12",
      leftMotorZpp: "T11",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 68,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 3. Given that I enter all the values for case 14, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "2",
        leftMotor: "1"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "2",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "1",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "44",
      rightTouchContainsNt: false,
      leftTouchTotal: "45",
      leftTouchContainsNt: false,
      rightPrickTotal: "43",
      rightPrickContainsNt: false,
      leftPrickTotal: "46",
      leftPrickContainsNt: false,
      rightMotorTotal: "27",
      leftMotorTotal: "26",
      upperMotorTotal: "50",
      lowerMotorTotal: "3",
      touchTotal: "89",
      prickTotal: "89",
      rightSensory: "L1",
      leftSensory: "L2",
      rightMotor: "L1",
      leftMotor: "L1",
      neurologicalLevelOfInjury: "L1",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "L2",
      leftMotorZpp: "L2",
      asiaImpairmentScale: "B"
    }
  },
  {
    id: 69,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 4. Given that I enter all the values for case 15, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "10",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "10",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "10",
      rightTouchContainsNt: false,
      leftTouchTotal: "10",
      leftTouchContainsNt: false,
      rightPrickTotal: "10",
      rightPrickContainsNt: false,
      leftPrickTotal: "10",
      leftPrickContainsNt: false,
      rightMotorTotal: "10",
      leftMotorTotal: "10",
      upperMotorTotal: "20",
      lowerMotorTotal: "0",
      touchTotal: "20",
      prickTotal: "20",
      rightSensory: "C6",
      leftSensory: "C6",
      rightMotor: "C6",
      leftMotor: "C6",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "C6",
      leftSensoryZpp: "C6",
      rightMotorZpp: "C7",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 70,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 5. Given that I enter all the values for case 16, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "4",
        leftMotor: "4"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "2"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "1"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "1"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "2",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "4"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "3"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "4"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "4"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "4"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "9",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "9",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "9",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "19",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "17",
      rightTouchContainsNt: false,
      leftTouchTotal: "19",
      leftTouchContainsNt: false,
      rightPrickTotal: "8",
      rightPrickContainsNt: false,
      leftPrickTotal: "12",
      leftPrickContainsNt: false,
      rightMotorTotal: "18",
      leftMotorTotal: "28",
      upperMotorTotal: "18",
      lowerMotorTotal: "28",
      touchTotal: "36",
      prickTotal: "20",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C5",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 71,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 6. Given that I enter all the values for case 17, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "1",
        leftMotor: "2"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "1",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "3",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "38",
      leftTouchContainsNt: false,
      rightPrickTotal: "38",
      rightPrickContainsNt: false,
      leftPrickTotal: "38",
      leftPrickContainsNt: false,
      rightMotorTotal: "26",
      leftMotorTotal: "28",
      upperMotorTotal: "50",
      lowerMotorTotal: "4",
      touchTotal: "76",
      prickTotal: "76",
      rightSensory: "T12",
      leftSensory: "T12",
      rightMotor: "T12",
      leftMotor: "T12",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "T12",
      rightMotorZpp: "L2",
      leftMotorZpp: "L3",
      asiaImpairmentScale: "B"
    }
  },
  {
    id: 72,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 7. Given that I enter all the values for case 18, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "2",
        leftMotor: "4"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "1"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "3",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "16",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "24",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "16",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "21",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "32",
      rightTouchContainsNt: false,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "32",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "32",
      leftMotorTotal: "45",
      upperMotorTotal: "40",
      lowerMotorTotal: "37",
      touchTotal: "88",
      prickTotal: "88",
      rightSensory: "C5",
      leftSensory: "S4_5",
      rightMotor: "C6",
      leftMotor: "T1",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 73,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 8. Given that I enter all the values for case 19, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "2"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "1"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "4",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "38",
      rightTouchContainsNt: false,
      leftTouchTotal: "42",
      leftTouchContainsNt: false,
      rightPrickTotal: "38",
      rightPrickContainsNt: false,
      leftPrickTotal: "38",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "29",
      upperMotorTotal: "50",
      lowerMotorTotal: "4",
      touchTotal: "80",
      prickTotal: "76",
      rightSensory: "T12",
      leftSensory: "T12",
      rightMotor: "T12",
      leftMotor: "T12",
      neurologicalLevelOfInjury: "T12",
      rightSensoryZpp: "T12",
      leftSensoryZpp: "L4",
      rightMotorZpp: "T12",
      leftMotorZpp: "L4",
      asiaImpairmentScale: "C"
    }
  },
  {
    id: 74,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 9. Given that I enter all the values for case 20, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "5",
        leftMotor: "4"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "1"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "1"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "1"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "4",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      }
    },
    totals: {
      rightUpperMotorTotal: "24",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "19",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "23",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "2",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "42",
      rightTouchContainsNt: false,
      leftTouchTotal: "44",
      leftTouchContainsNt: false,
      rightPrickTotal: "19",
      rightPrickContainsNt: false,
      leftPrickTotal: "26",
      leftPrickContainsNt: false,
      rightMotorTotal: "47",
      leftMotorTotal: "21",
      upperMotorTotal: "43",
      lowerMotorTotal: "25",
      touchTotal: "86",
      prickTotal: "45",
      rightSensory: "C7",
      leftSensory: "C6",
      rightMotor: "T1",
      leftMotor: "C8",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "L3",
      asiaImpairmentScale: "D"
    }
  },
  {
    id: 75,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 10. Given that I enter all the values for case 21, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "4"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4",
        leftMotor: "4"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "0",
        rightMotor: "4",
        leftMotor: "4"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "2",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "3",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "18",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "12",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "15",
      rightTouchContainsNt: false,
      leftTouchTotal: "9",
      leftTouchContainsNt: false,
      rightPrickTotal: "11",
      rightPrickContainsNt: false,
      leftPrickTotal: "10",
      leftPrickContainsNt: false,
      rightMotorTotal: "18",
      leftMotorTotal: "12",
      upperMotorTotal: "30",
      lowerMotorTotal: "0",
      touchTotal: "24",
      prickTotal: "21",
      rightSensory: "C6",
      leftSensory: "C4",
      rightMotor: "C6",
      leftMotor: "C5",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "T1",
      leftSensoryZpp: "C7",
      rightMotorZpp: "T1",
      leftMotorZpp: "C7",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 76,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Edge case 11. Given that I enter all the values for case 22, then the algorighmt calculates the correct results",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "56",
      rightTouchContainsNt: false,
      leftTouchTotal: "56",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "50",
      touchTotal: "112",
      prickTotal: "112",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 77,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "C8",
    leftLowestNonKeyMuscleWithMotorFunction: "T5",
    comments: "Given that I complete a form with all values set to Zero, I get an ASIA impairment of C1",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      C8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      T2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "0",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "0",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "6",
      rightTouchContainsNt: false,
      leftTouchTotal: "6",
      leftTouchContainsNt: false,
      rightPrickTotal: "6",
      rightPrickContainsNt: false,
      leftPrickTotal: "6",
      leftPrickContainsNt: false,
      rightMotorTotal: "0",
      leftMotorTotal: "0",
      upperMotorTotal: "0",
      lowerMotorTotal: "0",
      touchTotal: "12",
      prickTotal: "12",
      rightSensory: "C4",
      leftSensory: "C4",
      rightMotor: "C4",
      leftMotor: "C4",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "C4",
      leftSensoryZpp: "C4",
      rightMotorZpp: "C8",
      leftMotorZpp: "T5",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 78,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "",
    leftLowestNonKeyMuscleWithMotorFunction: "L1",
    comments: "Feature requested by Ron Reeves. When expressing a non-key muscle with motor function through the dropdowns, the original code was not taking into consideration its value when calculating the Zone of Partial Preservation. If the value is the lowest Motor value then it will be used to calculate the ZPP. Given that I complete a form for a complete injury with motor values all the way to T1 and I select L1 as the left lowest non-key muscle with motor value WHEN I press the calculate button THEN the resulting right motor ZPP value is L1\n Left motor function at 20",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "4"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "18",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "22",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "33",
      rightTouchContainsNt: false,
      leftTouchTotal: "19",
      leftTouchContainsNt: false,
      rightPrickTotal: "14",
      rightPrickContainsNt: false,
      leftPrickTotal: "16",
      leftPrickContainsNt: false,
      rightMotorTotal: "18",
      leftMotorTotal: "22",
      upperMotorTotal: "40",
      lowerMotorTotal: "0",
      touchTotal: "52",
      prickTotal: "30",
      rightSensory: "C7",
      leftSensory: "C6",
      rightMotor: "C7",
      leftMotor: "C8",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "S3",
      leftSensoryZpp: "T9",
      rightMotorZpp: "T1",
      leftMotorZpp: "L1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 79,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "L1",
    leftLowestNonKeyMuscleWithMotorFunction: "",
    comments: "Feature requested by Ron Reeves. When expressing a non-key muscle with motor function through the dropdowns, the original code was not taking into consideration its value when calculating the Zone of Partial Preservation. If the value is the lowest Motor value then it will be used to calculate the ZPP. Given that I complete a form for a complete injury with motor values all the way to T1 and I select L1 as the right lowest non-key muscle with motor value WHEN I press the calculate button THEN the resulting right motor ZPP value is L1.\n Right motor function at 20",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "4",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "3",
        leftMotor: "4"
      },
      T1: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "1",
        leftMotor: "3"
      },
      T2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T4: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T8: {
        rightTouch: "1",
        leftTouch: "1",
        rightPrick: "0",
        leftPrick: "1"
      },
      T9: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "1"
      },
      T10: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "1",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S4_5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      }
    },
    totals: {
      rightUpperMotorTotal: "18",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "22",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "33",
      rightTouchContainsNt: false,
      leftTouchTotal: "19",
      leftTouchContainsNt: false,
      rightPrickTotal: "14",
      rightPrickContainsNt: false,
      leftPrickTotal: "16",
      leftPrickContainsNt: false,
      rightMotorTotal: "18",
      leftMotorTotal: "22",
      upperMotorTotal: "40",
      lowerMotorTotal: "0",
      touchTotal: "52",
      prickTotal: "30",
      rightSensory: "C7",
      leftSensory: "C6",
      rightMotor: "C7",
      leftMotor: "C8",
      neurologicalLevelOfInjury: "C6",
      rightSensoryZpp: "S3",
      leftSensoryZpp: "T9",
      rightMotorZpp: "L1",
      leftMotorZpp: "T1",
      asiaImpairmentScale: "A"
    }
  },
  {
    id: 80,
    group: "OriginalCases",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Use of both the Asterisk and the Exclamation point flag on the same form",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "1!",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "0!",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "4!",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5*"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "24!",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "25",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "54!",
      rightTouchContainsNt: false,
      leftTouchTotal: "55!",
      leftTouchContainsNt: false,
      rightPrickTotal: "56",
      rightPrickContainsNt: false,
      leftPrickTotal: "56",
      leftPrickContainsNt: false,
      rightMotorTotal: "49!",
      leftMotorTotal: "50",
      upperMotorTotal: "50",
      lowerMotorTotal: "49!",
      touchTotal: "109!",
      prickTotal: "112",
      rightSensory: "S4_5",
      leftSensory: "S4_5",
      rightMotor: "S4_5",
      leftMotor: "S4_5",
      neurologicalLevelOfInjury: "S4_5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "",
      leftMotorZpp: "",
      asiaImpairmentScale: "E"
    }
  },
  {
    id: 81,
    group: "RevisionsToTheLogic",
    examDate: "0001-01-01T00:00:00",
    analContraction: "No",
    analSensation: "No",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Motor function present in non-key muscle at T1 on the right and at T1 on the left.\n It identifies this as L motor T3 but is it not actually T1 (as T1 motor testing was 5;\n with T3 is normal sensation but above this sensation is abnormal).",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "1",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "0",
        leftPrick: "0"
      },
      T6: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T7: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T8: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T9: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T10: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T11: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      T12: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      L2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L3: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L4: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      L5: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S1: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0",
        rightMotor: "0",
        leftMotor: "0"
      },
      S2: {
        rightTouch: "0",
        leftTouch: "0",
        rightPrick: "0",
        leftPrick: "0"
      },
      S3: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      },
      S4_5: {
        rightTouch: "0!",
        leftTouch: "0!",
        rightPrick: "0!",
        leftPrick: "0!"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "0",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "0",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "23!",
      rightTouchContainsNt: false,
      leftTouchTotal: "24!",
      leftTouchContainsNt: false,
      rightPrickTotal: "15!",
      rightPrickContainsNt: false,
      leftPrickTotal: "16!",
      leftPrickContainsNt: false,
      rightMotorTotal: "25",
      leftMotorTotal: "25",
      upperMotorTotal: "50",
      lowerMotorTotal: "0",
      touchTotal: "47!",
      prickTotal: "31!",
      rightSensory: "C4",
      leftSensory: "C5",
      rightMotor: "T1",
      leftMotor: "T1",
      neurologicalLevelOfInjury: "C4",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "T1",
      leftMotorZpp: "T1",
      asiaImpairmentScale: "B"
    }
  },
  {
    id: 82,
    group: "RevisionsToTheLogic",
    examDate: "0001-01-01T00:00:00",
    analContraction: "Yes",
    analSensation: "Yes",
    rightLowestNonKeyMuscleWithMotorFunction: "0",
    leftLowestNonKeyMuscleWithMotorFunction: "0",
    comments: "Motor function present in non-key muscle at T1 on the right and at T1 on the left.\n It identifies this as L motor T3 but is it not actually T1 (as T1 motor testing was 5;\n with T3 is normal sensation but above this sensation is abnormal).",
    dermatomes: {
      C2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      C5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C6: {
        rightTouch: "1",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      C8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "1",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      T2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T6: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T7: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T8: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T9: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T10: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T11: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      T12: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      L2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L3: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L4: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      L5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "4"
      },
      S1: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2",
        rightMotor: "5",
        leftMotor: "5"
      },
      S2: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      },
      S3: {
        rightTouch: "2",
        leftTouch: "1",
        rightPrick: "2",
        leftPrick: "1"
      },
      S4_5: {
        rightTouch: "2",
        leftTouch: "2",
        rightPrick: "2",
        leftPrick: "2"
      }
    },
    totals: {
      rightUpperMotorTotal: "25",
      rightUpperMotorContainsNt: false,
      leftUpperMotorTotal: "25",
      leftUpperMotorContainsNt: false,
      rightLowerMotorTotal: "25",
      rightLowerMotorContainsNt: false,
      leftLowerMotorTotal: "24",
      leftLowerMotorContainsNt: false,
      rightTouchTotal: "55",
      rightTouchContainsNt: false,
      leftTouchTotal: "55",
      leftTouchContainsNt: false,
      rightPrickTotal: "54",
      rightPrickContainsNt: false,
      leftPrickTotal: "55",
      leftPrickContainsNt: false,
      rightMotorTotal: "50",
      leftMotorTotal: "49",
      upperMotorTotal: "50",
      lowerMotorTotal: "49",
      touchTotal: "110",
      prickTotal: "109",
      rightSensory: "C5",
      leftSensory: "S2",
      rightMotor: "T1",
      leftMotor: "L5",
      neurologicalLevelOfInjury: "C5",
      rightSensoryZpp: "",
      leftSensoryZpp: "",
      rightMotorZpp: "S1",
      leftMotorZpp: "S1",
      asiaImpairmentScale: "D"
    }
  }
];